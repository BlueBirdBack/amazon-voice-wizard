...
class ChunkedHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        print("[START]: Received GET for %s with query: %s" % (path, query))
    ...
    def route_index(self, path, query):
        ...
        try:
            return ResponseData(
                ...
                # HTML file
                data_stream=open(os.path.join(sys.path[0], path[1:]), "rb"),
            )
    ...
    def route_read(self, path, query):
        ...
        speed = self.query_get(query, "speed", "100%")
        engine = self.query_get(query, "engine", "neural")  # generative

        # Map the speed options to SSML prosody rate values
        speed_mapping = {
            "xxx-slow": "10%",
            "xx-slow": "25%",
            "x-slow": "50%",
        ...
        if len(text) == 0 or len(voiceId) == 0 or outputFormat not in AUDIO_FORMATS:
            ...
            try:
                # Wrap the text in SSML tags to control the speaking rate
                ssml_text = (
                    f'<speak><prosody rate="{ssml_speed}">{text}</prosody></speak>'
                )
                ...
                if "InvalidSsmlException" in str(err):
                    # If SSML is not supported, fall back to plain text and standard engine
                    try:
                        response = polly.synthesize_speech(
                            Text=text,
                            VoiceId=voiceId,
                        ...
                        raise HTTPStatusError(
                            HTTP_STATUS["INTERNAL_SERVER_ERROR"], str(fallback_err)
                        )
                else:
                    raise HTTPStatusError(
...
if __name__ == "__main__":
    ...
    server = ThreadedHTTPServer(
        (arguments.host, arguments.port), ChunkedHTTPRequestHandler
    )
    print("Starting server, use <Ctrl-C> to stop...")
    print(
        "Open {0}://{1}:{2}{3} in a web browser.".format(
            PROTOCOL, arguments.host, arguments.port, ROUTE_INDEX
        )
    )
